...
def py_cmp_fun(a, b):
    ...
    if fa.contents.src_ip < fb.contents.src_ip:
        return -1
    elif fa.contents.src_ip > fb.contents.src_ip:
        return 1
    if fa.contents.src_port < fb.contents.src_port:
        return -1
    elif fa.contents.src_port > fb.contents.src_port:
        return 1
    if fa.contents.dst_ip < fb.contents.dst_ip:
        return -1
    elif fa.contents.dst_ip > fb.contents.dst_ip:
        return 1
    if fa.contents.dst_port < fb.contents.dst_port:
        return -1
    elif fa.contents.dst_port > fb.contents.dst_port:
        return 1
    if fa.contents.protocol < fb.contents.protocol:
        return -1
    elif fa.contents.protocol > fb.contents.protocol:
        return 1
    return 0
...
count_protocol = (c_int32 * (ndpi.ndpi_wrap_ndpi_max_supported_protocols() +
                             ndpi.ndpi_wrap_ndpi_max_num_custom_protocols() + 1))()
...
def packetcaptured(pkt):
    ...
    if int(pkt[1].frag) == 0:  # not fragmented packet
        ...
        if flow.contents.detection_completed == 0:
            ...
            if flow.contents.protocol == 6:
                valid = flow.contents.packets > max_num_tcp_dissected_pkts
            elif flow.contents.protocol == 17:
                valid = flow.contents.packets > max_num_udp_dissected_pkts

            # should we continue anylizing packet or not?
            if valid or flow1.app_protocol != 0:
                if valid or flow1.master_protocol != 91:  # or # 91 is NDPI_PROTOCOL_TLS
                    flow.contents.detection_completed = 1  # protocol found
                    if flow1.app_protocol == 0:
