...
echo "Labelling Log" > labelling/labelling_log.txt # log file
...
do
     ...
     if [ $remainder -eq 0 ]
     then
          echo "`date` - TCP: ($tcp_file_counter/$num_tcp_files), UDP: ($udp_file_counter/$num_udp_files)" >> labelling/labelling_log.txt
     fi
